...
// while g_bThreadedMode is TRUE.  Consumed by PAL_ProcessEvent() on the
// logic thread and by PAL_InputSnapshot().
#  if SDL_VERSION_ATLEAST(3,0,0)
...
/*++
  Purpose:
    ...
    In threaded mode writes to the atomic accumulator so that
    one-shot keys survive PAL_ClearKeyState() on the logic thread.
...
--*/
{
   ...
   // the shared state using dimensional-overwrite semantics.
   //
   INT iKeys;
   do {
...
--*/
{
   ...
   // The logic thread consumes accumulated keys from the atomic so that
   // sub-loops (menus, dialogues, etc.) see input without stalling.
   //
#if SDL_VERSION_ATLEAST(2,0,0)
